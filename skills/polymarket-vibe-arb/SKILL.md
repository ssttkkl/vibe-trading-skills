...
- **分析指令（方法论指导）**：
  ...
  - **价格盘**：用期权隐含波动率算概率。注意当前价在范围中的位置（中心 vs 边界）。如果一个事件有多个到期日，对比价格差反推当前状态
  - **地缘/系列盘**：如果同一事件有多个过期子市场，去查它们为什么全部判 No——基本面的哪些因素导致每月都 No？这些因素在本次到期前是否依然成立？**不是重复历史，而是分析基本面是否改变**
