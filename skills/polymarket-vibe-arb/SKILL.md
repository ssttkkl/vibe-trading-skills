...
- **候选盘按类型分组**，只附原始数据
- **分析指令（方法论指导）**：
  - **规则必查**：找到每个盘口的 Resolution Rules 原文，检查时间窗陷阱（规则写的年份和到期日是否一致）。评估规则客观性：数据驱动 > 物理事实 > 主观判断
  - **价格盘**：用期权隐含波动率算概率。注意当前价在范围中的位置（中心 vs 边界）。如果一个事件有多个到期日，对比价格差反推当前状态——同一系列多个月份全部判 No 是极强信号
  - **地缘盘**：找最新新闻确认局势。注意事件是否有同系列的历史子市场全部判 No（模式验证）
  - **通用**：确认市场定价和你算出的概率之间的差距是否足够大。用户资金 $400，剩余 $100 未部署——确定性优先于 ROI，薄盘要标注滑点风险
- **输出格式**：P0 确定 / P1 价值 / 排除 + $100 分配建议
...
1. **"XX 事件 6/30 前不会发生"系列** — 多个月份全部判 No（Pahlavi、Tim Walz 辞职、中国入侵台湾）
2. **"XX 公司全球最大"系列** — 几天内不可能被反超
3. **"XX 人在 6/30 前进入伊朗"系列** — Trump/Pete Hegseth/Netanyahu=概率 0
4. **地缘政治量化指标盘** — IMF Portwatch 等客观数据源
